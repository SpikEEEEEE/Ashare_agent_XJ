...
from ashare_agent.selection.deepseek_features import DeepSeekFeatureGenerator
from ashare_agent.selection.demo import generate_demo_data
from ashare_agent.selection.features import build_features
from ashare_agent.selection.generated_features import (
    FeatureDefinition,
...
class GeneratedFeatureTest(unittest.TestCase):
    ...
    def test_screening_removes_redundant_features_and_writes_outputs(self) -> None:
        ...
        try:
            ...
            prepared = selector.prepare(self.market)
            with self.assertRaisesRegex(ValueError, "information unavailable"):
                selector.select_prepared(
                    prepared,
                    as_of=result.label_data_end,
                )
            selection = selector.select_prepared(prepared)
