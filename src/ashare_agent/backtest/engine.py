...
class PortfolioBacktester:
    ...
    def __init__(
        ...
        *,
        ...
        if profile.default_board_lot is None:
            raise ValueError(
                "This backtest engine requires a fixed market board lot"
            )
        self.board_lot = profile.default_board_lot
        self.data_feed = data_feed
        self.decision_engine = decision_engine
        if risk_policy is None:
            from ashare_agent.adapters.risk_policy_factory import (
                build_risk_policy,
            )
    ...
    def _affordable_buy_shares(
        ...
        *,
        ...
        lots = max(0, requested // self.board_lot)
        while lots > 0:
            shares = lots * self.board_lot
    ...
    def _buy(
        ...
        *,
        ...
        if requested < self.board_lot or budget <= 0:
            return None, budget
        execution_price = self._execution_price(
            reference_open,
            "buy",
