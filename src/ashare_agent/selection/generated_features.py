...
def load_feature_availability_date(path: str | Path) -> pd.Timestamp:
    """Return the first date after which a feature-selection artifact is usable."""
    feature_path = Path(path)
    payload = json.loads(feature_path.read_text(encoding="utf-8"))
    if not isinstance(payload, dict):
        raise FeatureDefinitionError(
            "Configured generated features require point-in-time lineage"
        )
    screening = payload.get("screening")
    raw_date = (
        screening.get("label_data_end")
        if isinstance(screening, dict)
        else payload.get("available_from")
    )
    parsed = pd.to_datetime(raw_date, errors="coerce")
    if pd.isna(parsed):
        raise FeatureDefinitionError(
            "Generated feature artifact must declare "
            "screening.label_data_end or available_from"
        )
