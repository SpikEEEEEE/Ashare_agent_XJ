...
from ashare_agent.core.config import Settings, has_configured_secret
from ashare_agent.adapters.selection_data import (
    TushareSelectionDataProvider,
    build_selection_data_provider,
)
...
def test_selection_provider_receives_tushare_token_loaded_from_dotenv(
    tmp_path,
    monkeypatch,
):
    monkeypatch.delenv("TUSHARE_TOKEN", raising=False)
    (tmp_path / ".env").write_text(
        "TUSHARE_TOKEN=dotenv-test-token\n",
        encoding="utf-8",
    )

    settings = Settings.from_env(tmp_path)
    provider = build_selection_data_provider(settings, AppConfig())

    assert isinstance(provider, TushareSelectionDataProvider)
    assert provider.token == "dotenv-test-token"
    assert provider.allow_network is True
    assert provider.timeout_seconds == settings.tushare_timeout_seconds

    offline_provider = build_selection_data_provider(
        replace(settings, data_mode="offline_only"),
        AppConfig(),
    )
    assert isinstance(offline_provider, TushareSelectionDataProvider)
