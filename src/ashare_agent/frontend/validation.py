...
def normalize_symbol(value: Any, *, market: str = "CN") -> str:
    symbol = str(value or "").strip().upper()
    try:
        return get_market_profile(market).normalize_provider_symbol(symbol)
    except ValueError as exc:
        raise InputValidationError(
            f"股票代码“{symbol or '(空)'}”不属于 {market.upper()} 市场。"
        ) from exc


def parse_universe(value: str, *, market: str = "CN") -> list[str]:
    raw_symbols = [
        item
    ...
    symbols = [
        normalize_symbol(item, market=market)
...
def parse_positions(
    ...
    *,
    ...
    for row_number, row in enumerate(rows, start=1):
        ...
        symbol = normalize_symbol(symbol_value, market=market)
        shares = _integer(
            row.get("shares"),
            field=f"{symbol} 持有股数",
            minimum=1,
