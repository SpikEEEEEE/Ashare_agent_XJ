...
class AShareRiskPolicy:
    ...
    def __init__(self, settings: Settings) -> None:
        ...
        if profile.default_board_lot is None:
            raise ValueError(
                "AShareRiskPolicy requires a fixed market board lot"
            )
        self.board_lot = profile.default_board_lot
    ...
    def apply(
        ...
        for symbol in decision_input.symbols:
            ...
            if theoretical_target > current_shares:
                buy_delta = (
                    (theoretical_target - current_shares) // self.board_lot
                ) * self.board_lot
        ...
        for plan in buy_plans:
            ...
            if requested_cost > remaining_cash:
                ...
                affordable_lots = int(
                    remaining_cash
                    / (Decimal(board_lot) * price * fee_multiplier)
                )
                allowed_delta = max(0, affordable_lots * board_lot)
                plan["adjustments"].append(
                    {
                        "rule": "AVAILABLE_CASH",
        ...
        return {
            ...
            "provider": decision_input.data_provider,
