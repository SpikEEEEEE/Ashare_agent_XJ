...
生产选股继续以 Tushare 为规范市场数据源。LightGBM 使用严格时点标签训练并生成
可配置的量化预选池；随后可调用一次结构化 LLM 做候选池内横向复核。复核器不能
引入池外股票，只能读取传入的量化证据。它的分数按置信度向中性收缩，再以不超过
配置上限的权重与量化排序混合，最后重新执行行业上限约束。Provider 或输出 schema
失败时，根据配置安全失败或回退量化排序。

`CandidatePool` 仍然不可变。后续选股运行会利用已经下载的 Tushare 数据，为旧
候选池写入独立、不可变的 T+N 评估快照，比较复核名单和原始量化 Top-K，并记录
收益增益与排序 IC。这些结果是可审计的评估证据，不是可被 LLM 任意改写的记忆，
也不会未经验证自动修改模型参数。
