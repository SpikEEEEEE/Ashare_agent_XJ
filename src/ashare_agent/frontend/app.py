...
from ashare_agent.frontend.api_client import AdvisorApi, BackendError
from ashare_agent.frontend.components import (
    history_label,
    inject_styles,
    render_decision_run,
    render_hero,
...
def _sidebar() -> None:
    with st.sidebar:
        ...
        st.text_input(
            ...
            key="api_url",
            disabled=(
                os.getenv("ADVISOR_ALLOW_CUSTOM_API_URL", "false")
                .strip()
                .lower()
                not in {"1", "true", "yes"}
            ),
...
def _load_default_universe(api: AdvisorApi, *, force: bool = False) -> None:
    ...
    try:
        ...
        st.session_state.market_id = str(payload.get("market") or "CN").upper()
    except BackendError as exc:
    ...
    with st.form("portfolio_advice_form", clear_on_submit=False):
        ...
        with top_right:
            mode_label = st.selectbox(
                ...
                help=(
                    "智能选股会先运行 LightGBM 横截面选择器，再在候选股票"
                    "和当前持仓的并集中寻找机会；"
                    "仅分析当前持仓不会提出新股票。"
                ),
            )
    ...
    if submitted:
        try:
            ...
            positions = parse_positions(
                positions_table.to_dict("records"),
                market=market_id,
            )
            mode = (
                "holdings_only"
                if mode_label == "仅分析当前持仓"
                else "rebalance"
            )
            if mode == "holdings_only" and not positions:
                raise InputValidationError("仅分析当前持仓模式至少需要一条持仓。")
            universe = (
                parse_universe(universe_text, market=market_id)
                if mode_label == "自定义股票池再平衡"
                else None
            )
            universe_source = {
                "智能选股后再平衡": "fresh_selection",
                "服务器固定股票池": "static",
                "自定义股票池再平衡": "static",
                "仅分析当前持仓": "static",
            }[mode_label]
