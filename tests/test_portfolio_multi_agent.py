...
from ashare_agent.adapters.openai_decision import OpenAIDecisionEngine
...
from ashare_agent.agents.portfolio_allocator import DeterministicPortfolioAllocator
from ashare_agent.agents.portfolio_graph import (
    PortfolioAgentGraph,
    PortfolioAgentGraphError,
)
from ashare_agent.agents.portfolio_schemas import PoolAnalystReport, PortfolioProposal
...
def test_all_analysts_receive_candidate_selection_metadata(tmp_path):
    agents = ScriptedPortfolioAgents()
    selection = {
        ...
        "rank": 1,
        "score": 0.923,
        "reason": "top_cross_sectional_score",
        "signals": {
    ...
    decision_input = replace(
        ...
        candidate_pool_id="pool_cn_2026-07-21_test",
        candidate_metadata={"300750.SZ": selection},
    )
    engine = PortfolioMultiAgentDecisionEngine(
        make_settings(tmp_path),
        agent_client=agents,
    ...
    for role in ("technical", "fundamental", "news"):
        ...
        assert symbols["300750.SZ"]["selection"] == {
            "is_candidate": True,
            **selection,
        }
        assert symbols["600519.SH"]["selection"] == {
            "is_candidate": False,
            "reason": "existing_holding_outside_candidate_pool",
        }
