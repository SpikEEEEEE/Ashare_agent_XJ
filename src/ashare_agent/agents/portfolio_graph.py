...
from ashare_agent.domain.features import build_symbol_context
from ashare_agent.domain.instruments import get_market_profile
from ashare_agent.domain.models import DecisionInput
...
def _safe_exception_summary(exc: Exception) -> str:
    ...
    if (
        ...
        != "ashare_agent.adapters.portfolio_multi_agent"
    ):
...
class PortfolioAgentGraph:
    ...
    def _constraints(self) -> dict[str, Any]:
        profile = get_market_profile(self.settings.active_market)
        return {
            "market": profile.market_id,
            "currency": profile.currency,
            "minimum_cash_ratio": float(self.settings.min_cash_ratio),
            "maximum_position_ratio": float(self.settings.max_position_ratio),
            "maximum_positions": self.settings.max_positions,
            "buy_board_lot": (
                profile.default_board_lot
                if profile.default_board_lot is not None
                else "instrument_master"
            ),
            "advisory_only": True,
        }
    ...
    def _run_analysts(self, state: PortfolioAgentState) -> None:
        market = state.decision_input.market_id
        role_prompts = {
            "technical": (
                f"You are the cross-sectional technical analyst for a {market} equity pool. "
            ...
            "fundamental": (
                f"You are the cross-sectional fundamental analyst for a {market} equity pool. "
            ...
            "news": (
                f"You are the cross-sectional news and sentiment analyst for a {market} equity "
    ...
    def _run_research_manager(self, state: PortfolioAgentState) -> None:
        ...
        state.research_plan = self._invoke_validated(
            ...
            system_prompt=(
                f"You are the research manager for a {state.decision_input.market_id} "
                "equity portfolio. Adjudicate the "
