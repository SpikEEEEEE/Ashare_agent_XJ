...
def _coerce_bool(series: pd.Series) -> pd.Series:
    ...
    if pd.api.types.is_numeric_dtype(series):
        ...
        if invalid.any():
            values = sorted(set(numeric.loc[invalid].dropna().astype(str)))
            raise ValueError(f"Unrecognized boolean values: {values[:10]}")
        return numeric.astype(int).astype(bool)

    normalized = series.astype("string").str.strip().str.lower()
    true_values = {"1", "true", "t", "yes", "y", "是"}
    false_values = {"0", "false", "f", "no", "n", "否"}
...
def load_market_data(path: str | Path, config: AppConfig) -> pd.DataFrame:
    ...
    missing_status = [
        column
        for column in ELIGIBILITY_STATUS_COLUMNS
        if column not in frame.columns
    ]
    if missing_status:
        raise ValueError(
            "Missing required eligibility status columns: "
            f"{missing_status}. CSV selection data must explicitly declare "
            "ST, suspension, and price-limit status."
        )
    for column in ELIGIBILITY_STATUS_COLUMNS:
        values = frame[column]
        ...
        if missing_values.any():
            examples = frame.index[missing_values].tolist()[:5]
            raise ValueError(
                f"Eligibility status column '{column}' cannot contain missing "
                f"values; invalid rows: {examples}"
            )
