...
def validate_config(config: AppConfig) -> None:
    def validate_windows(name: str, values: list[int]) -> None:
        if (
            not values
            or any(
            ...
            raise ValueError(
                f"{name} must contain unique positive integer windows"
            )

    if config.features.prediction_horizon < 1:
        raise ValueError("prediction_horizon must be at least 1")
    validate_windows("momentum_windows", config.features.momentum_windows)
