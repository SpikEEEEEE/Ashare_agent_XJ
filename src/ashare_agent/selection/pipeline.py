...
class CandidateSelector:
    ...
    def prepare(self, market_data: pd.DataFrame) -> PreparedData:
        ...
        if missing:
            raise ValueError(f"Canonical market data is missing columns: {missing}")
        frame = market_data.copy()
        frame["date"] = pd.to_datetime(frame["date"], errors="coerce")
        if frame["date"].isna().any():
            raise ValueError("Canonical market data contains invalid dates")
        if frame.duplicated(["date", "code"]).any():
            raise ValueError("Canonical market data contains duplicate date/code rows")
        frame = frame.sort_values(
            ["code", "date"],
            kind="stable",
        ).reset_index(drop=True)
        return build_features(frame, self.config)
    ...
    def _resolve_score_date(
        ...
        availability = prepared.generated_features_available_after
        if availability is not None and score_date <= availability:
            raise ValueError(
                "Generated features were selected with information unavailable "
                f"at {score_date.date()}; use a score date after "
                f"{availability.date()}"
            )
